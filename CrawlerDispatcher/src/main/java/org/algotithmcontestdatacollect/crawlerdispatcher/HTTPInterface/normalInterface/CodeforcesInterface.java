...
public class CodeforcesInterface {
    ...
    public String getStuContest(@PathVariable Long uid, HttpServletRequest request, HttpServletResponse response){
        ...
        try{
            ...
            try{
                List<CfAccount> entities = cfAccountRepository.getCfAccountEntitiesByUid(uid); // 根据Uid获取整个Cf实体,这里使用了JPA与数据库连接，如果不会JPA请参考https://www.bilibili.com/video/BV1F54y1B7Gy
...
//            List<String> cfids = new ArrayList<>();
            Long cid;
            try {
                CfSubmit entity = cfSubmitRepository.getCfSubmitEntityBySid(sid); // 根据Uid获取整个Submit实体,这里使用了JPA与数据库连接，如果不会JPA请参考https://www.bilibili.com/video/BV1F54y1B7Gy
//                for(var entity:entities){
//                    cfids.add(entity.getCodeforcesId());
//                }
                cid = entity.getCid();
