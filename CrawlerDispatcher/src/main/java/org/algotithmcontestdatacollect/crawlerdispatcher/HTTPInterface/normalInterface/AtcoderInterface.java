package org.algotithmcontestdatacollect.crawlerdispatcher.HTTPInterface.normalInterface;
...
public class AtcoderInterface {
    ...
    public String getContestSubmit(@PathVariable Long uid, @PathVariable Long cid, HttpServletRequest request, HttpServletResponse response){
        ...
        try{
            ...
            try{
                List<AcAccount> entities = acAccountRepository.getAcAccountsEntitiesByUid(uid); // 根据Uid获取整个Ac实体,这里使用了JPA与数据库连接，如果不会JPA请参考https://www.bilibili.com/video/BV1F54y1B7Gy
    ...
    public String stuContest(@PathVariable Long uid, HttpServletRequest request, HttpServletResponse response){
        ...
        try{
            ...
            List<String> acids = new ArrayList<>();
            try{
                List<AcAccount> entities = acAccountRepository.getAcAccountsEntitiesByUid(uid); // 根据Uid获取整个Cf实体,这里使用了JPA与数据库连接，如果不会JPA请参考https://www.bilibili.com/video/BV1F54y1B7Gy
                for(var entity:entities){
                    acids.add(entity.getAtcoderId());
                }
            }catch (Exception e){
    ...
    public String getSubmitCode(@PathVariable Long sid, HttpServletRequest request, HttpServletResponse response){
        ...
        try{
            ...
            try{
                AcSubmit entity = acSubmitRepository.getAcSubmitEntityBySid(sid); // 根据Uid获取整个Ac实体,这里使用了JPA与数据库连接，如果不会JPA请参考https://www.bilibili.com/video/BV1F54y1B7Gy
