...
public class AtcoderCron {
    ...
    public void getStuContest() {
        ...
        try {
            List<AcAccountWithUsername> entities = userAccountService.getAcAccounts();
    ...
    public void getAllContestSubmit() {
        ...
        try {
            ...
            List<AcAccountWithUsername> entities = null;
            List<AcContest> contests = acContestRepository.findAll(Sort.by(Sort.Direction.DESC, "startTimeStamp"));
            try {
                LOGGER.info("获得比赛记录");
    ...
    public void getRecent5StuContestSubmit() {
        List<AcContest> contests = acContestRepository.findAll(Sort.by(Sort.Direction.DESC, "startTimeStamp"));
        List<AcContest> toDownload = new ArrayList<>();
        for (var contest : contests) {
    ...
    public void getRandom5ContestSubmit() {
        List<AcContest> contests = acContestRepository.getAcContestEntitiesRandom(5);
        getNoExistContestSubmit(contests);
    }

    //    @Scheduled(cron = "0 20 * * * ?")
    public void get5NoSubmitRecordContest() {
        List<AcContest> contests = acContestRepository.getNoSubmitRecordContestEntities(5);
        getNoExistContestSubmit(contests);
    }

    //    @Scheduled(cron = "0 30 * * * ?")
    public void getAllExistStuContestButNoExistContestSubmit() {
        List<AcContest> contests = acContestRepository.findAll();
        List<AcAccountWithUsername> entities = userAccountService.getAcAccounts();
        ...
        List<AcStucontest> stucontests = acStucontestRepository.getNoSubmitStuContest();
    ...
    private void getNoExistContestSubmit(List<AcContest> contests) {
        List<AcAccountWithUsername> entities = userAccountService.getAcAccounts();
        for (var entity : entities) {
            for (var contest : contests) {
