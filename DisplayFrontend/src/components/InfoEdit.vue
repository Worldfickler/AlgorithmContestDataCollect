<template>
  <el-form :model="userinfo" :rules="rules" label-width="100px" ref="formRef">
    ...
    >
      <el-input
        v-model="userinfo.employInfo"
        :disabled="editStatus === false"
      />
    </el-form-item>
    <el-button @click="startEdit" v-if="editStatus === false"
      >修改信息</el-button
    >
    <el-button @click="cancel()" v-if="editStatus === true" :loading="loading"
      >取消</el-button
    >
    <el-button v-if="editStatus === true" @click="submit" :loading="loading"
      >提交</el-button
    >
  </el-form>
</template>
  ...
  employInfo: [
    {
      validator: (rule, value, callback) => {
        if (value === "" || (value.length > 0 && value.length <= 255)) {
          ...
          callback(new Error("输入内容不能超过255个字符!"));
        }
      },
      trigger: "blur",
    },
...
let startEdit = () => {
  ...
  userinfoTmp.value.employInfo = userinfo.value.employInfo;
...
let cancel = () => {
  ...
  userinfo.value.employInfo = userinfoTmp.value.employInfo;
    ...
    if (valid) {
      ...
      axiosUtil
        .updateMyInfo(
          ...
          userinfo.value.year,
          userinfo.value.stuno,
          userinfo.value.realname,
          userinfo.value.employInfo
        )
        .then((res) => {
          if (res.data.code === 200) {
