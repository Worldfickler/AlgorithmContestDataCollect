<template>
  <el-table :data="pageData" border>
    ...
    <el-table-column prop="atcoderId" label="账户名">
      <template #header="scope">
        {{scope.column.label}}
        <el-input v-model="atcoderIdFilter"></el-input>
      </template>
    </el-table-column>
...
const dspData = computed(()=>{
  ...
  dt = dt.filter((item)=>{
    ...
    if (atcoderIdFilter.value !== '' && !item.atcoderId.includes(atcoderIdFilter.value)) {
      return false
    }
...
onMounted(()=>{
  axiosUtil.getAllAtcoderAccount().then(res=> {
...
const deleteAccount = (id) => {
  ElMessageBox.confirm('此操作将永久删除该账户,并且其比赛数据与提交记录也会随之删除, 是否继续?', '提示', {
    ...
    axiosUtil.deleteAtcoderAccount(id).then(res=>{
      if(res.data.code === 200) {
